...
### A-37：不要になったメンバーのレコードをDBから削除する
- 会員番号とメールアドレスで削除会員を指定  
- 削除対象のメンバーが他の機能で何か使われていたらレコードの物理削除はしない

### A-38：DUPRレイティング情報を一括更新する
- DUPRからメンバーの情報を取得(画面のコピー＆ファイルへペースト)したファイルを読み込ませる
- DUPR IDが一致するメンバーのダブルスとシングルスのDUPR Ratingを更新する
- DUPR IDが一致しないメンバーの情報は何も変更しない
