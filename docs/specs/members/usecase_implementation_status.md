...
| **A-12** | 自分のプロフィールを編集する | ✅ 実装済 | `members/profile/edit`：ニックネーム・氏名ローマ字・郵便番号・住所・電話・プロフィールメモ・公開設定・緊急連絡先・DUPR ID/レートを編集可能。**パスワード変更は未実装**。会員番号・氏名・LINE ID は閲覧のみ。 |
...
| ID | ユースケース | 実装状況 | 備考 |
|----|-------------|----------|------|
| **A-21** | ホストを変更する | ❌ 未実装 | ゲストがホスト（紹介者）を変更する画面・フローなし。 |
...
- **未実装**: A-05（パスワードリセット）, A-19（他会員検索）, A-21（ホスト変更）  
