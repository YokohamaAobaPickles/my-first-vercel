 ...
 * Update  : 2026-03-19
 * Remarks : 
 * V3.9.0 - 追加：uploadProfileIcon（ストレージへの画像アップロード）を追加
...
/**
 * プロフィール画像を Supabase Storage にアップロードし、公開URLを取得する
 * @param memberId 会員ID（ファイル名に使用）
...
export const uploadProfileIcon = async (
  ...
  try {
    // 1. ファイル名の生成 (memberId + タイムスタンプ + 拡張子)
    const fileExt = file.name.split('.').pop();
    const fileName = `${memberId}_${Date.now()}.${fileExt}`;
    const filePath = `${fileName}`; // バケット直下に配置する場合

    // 2. Storageへアップロード (avatarsバケットを想定)
